...
function[FFT_result]=execute_fft_with_scilab_function(signal)
  FFT_result = fft(signal);
endfunction

// function to execute fft with dft build
function[FFT_result]=execute_fft_with_self_function(signal)
...
[Y_fungsi_scilab] = abs(execute_fft_with_scilab_function(y)); // perform fft
[Y_fungsi_sendiri] = abs(execute_fft_with_self_function(y)); // perform fft

//x_axis_reps=num_data;
x_axis_reps=0:SAMPLING_FREQUENCY;
clf;
subplot(1,3,1), plot_graph_2d(x,y,'Sinyal input','yes');
subplot(1,3,2), plot2d3('gnn',x_axis_reps,Y_fungsi_scilab);title('FFT fungsi scilab');
subplot(1,3,3), plot2d3('gnn',x_axis_reps,Y_fungsi_sendiri);title('FFT fungsi sendiri');
